...
Database.prototype.get = function get( key ){
    var self = this;
        self.log("Getting Key %s", key);

    return new Promise(function( resolve, reject ){
        self.connection.get( key, function(err, value){
            if ( err ) {
                return reject( err );
            } else {
                try {
...
Database.prototype.mget = function keys( keys ){
    var self = this;
        self.log("Getting mget %s", keys);

    return new Promise(function( resolve, reject ){
        var keysL = [];
        async.each( Utils.chunk( keys, 100 ), function( values, done ){
            self.connection.mget( values, function(err, values){
                keysL = keysL.concat( values );
                return done(err);
            });
        }, function(err){
            if ( err ){
                ...
                return resolve( keysL );
            }
        });
    });
}

Database.prototype.keys = function keys( key ){
    var self = this;
        self.log("Serching Key %s", key);

    return new Promise(function( resolve, reject ){
        self.connection.keys( key + ":*", function(err, value){
            if ( err ) {
                return reject( err );
            } else {
                return resolve( value );
...
Database.prototype.type = function type( key ){
    var self = this;
        self.log("Getting key:%s type", key);

    return new Promise(function( resolve, reject ){
        self.connection.type( key, function(err, type){
            if ( err ) {
                return reject( err );
            } else {
                return resolve( type );
...
Database.prototype.set = function set( key, value ){
    var self = this;
        self.log("Setting key:%s with value:%s", key, value);

    return new Promise(function( resolve, reject ){
        self.connection.set( key, value, function(err, value){
            if ( err ) {
                return reject( err );
            } else {
                return resolve( value );
...
Database.prototype.unset = function unset( key ){
      var self = this;
        self.log("Unsetting key:%s", key);

    return new Promise(function( resolve, reject ){
        self.connection.del( key, value, function(err, value){
            if ( err ) {
                return reject( err );
            } else {
                return resolve(true);
...
Database.prototype.order = function order( keys, criteria, primary ){
    criteria.where = {};

    return new Promise(function( resolve ){
        var data = {};
            data[primary] = keys;
        return resolve( WaterlineCriteria(primary, data, criteria).results );
    });

}

Database.prototype.applyWhere = function applyWhere( keys, where, primary ){
    var self = this;
    return new Promise(function( resolve ){
        keys = _.filter( keys, function( value, key ){
            var data = {};
                data[primary] = [self.schema.parse(primary, value)];
                return WaterlineCriteria(primary, data, where).results.length;
        });
         return resolve( keys );
    });
}
...
Database.prototype.applyModifiers = function applyModifiers( keys, modifiers, primary ){
    var self = this;
    return new Promise(function( resolve ){
        keys = _.filter( keys, function( value, key ){
            var data = {};
                data[primary] = [self.schema.parse(primary, value)];
                return WaterlineCriteria(primary, data, modifiers).results.length;
        });
         return resolve( keys );
    });
}
...
Database.prototype.find = function find(collectionName, criteria, cb) {
        ...
        var primary = this.schema._primary[collection];


        var limit = criteria.limit || Infinity;
        var offset = criteria.skip || 0;

        //lets log the search paramters...
        this.log("Searching %s With Criteria %j", criteria );

        // Find the index key used to keep track of all the keys in this collections
        var indexKey = this.schema.indexKey(collection, primary);
        var time = Date.now();
        ...
        //we need to do an "simple" query
        this.keys( indexKey )
            .then(function(values){
                return self.mget( values );
            })
            .then( function( keys ){
                return Promise.map( keys, function( value ){
                    return Promise.resolve(JSON.parse(value));
                });
            })
            .then(function( keys ){
                //apply the criteria on it now...
                return self.applyWhere( keys, criteria, collection );
            })
            .then( function( keys ){
                //lets order...
                return self.order( keys, criteria, collection );
            })
            .then(function(keys){
                self.log(Date.now() - time);
                return cb(null, _.slice( keys, offset, limit ));
            });
