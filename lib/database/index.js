...
Database.prototype.order = function order( keys, by, order ){
    var self = this;
        self.log("Ordering Keys");

    return new Promise(function( resolve ){
        return resolve( _.orderBy(keys, by, order));
    });

}

Database.prototype.applyWhere = function applyWhere( keys, where ){
    return new Promise(function( resolve ){
        return resolve( _.filter(keys, where));
    });
}
...
Database.prototype.find = function find(collectionName, criteria, cb) {
        ...
        if ( criteria.where ){
            //okay a more advanced search
            //... so this is where it gets a bit harder...
            this.keys( indexKey )
                ...
                .then(function( keys ){
                    ...
                    return self.applyWhere( keys, criteria.where);
                })
                .then( function( keys ){
                    //lets order...
                    return self.order( keys, [sort.row], [sort.type] );
                })
                .then(function(keys){
