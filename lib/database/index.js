...
Database.prototype.update = function update(collectionName, criteria, values, cb) {
  ...
  async.auto({
    ...
    updateIndexes: ['checkUniqueConstraints', 'checkPrimaryKeyValid', function(next, results) {
      ...
      function updateIndex(item, nextItem) {
        var indexKey = self.schema.indexKey(name, item);

        data.forEach(function(record) {
          var recordKey = self.schema.recordKey(name, item, data[item]);

          // Indexed value not updated
          if(!present(values[key])) return nextItem();

          // Remove the old index
          multi.srem(indexKey, Utils.sanitize(recordKey));

          // Set the new index value
          recordKey = self.schema.recordKey(name, item, values[item]);
          multi.sadd(index, Utils.sanitize(recordKey));
        });

        nextItem();
      }

      async.each(indiceNames, updateIndex, function(err) {
        if(err) return next(err);
