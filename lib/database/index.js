...
Database.prototype.drop = function drop(collectionName, relations, cb) {
  ...
  connection.client.smembers(index, function(err, values) {
    ...
    async.each(keys, function(key, next) {
      connection.client.del(key, next);
    }, cb);
  });
...
Database.prototype.find = function find(collectionName, criteria, cb) {
  ...
  if(Utils.present(criteria[primary])) {
    recordKey = this.schema.recordKey(name, primary, criteria);

    connection.client.get(recordKey, function(err, record) {
      if(err) return cb(err);
  ...
  async.auto({
    ...
    getSetMembers: function(next) {
      connection.client.smembers(indexKey, function(err, members) {
    ...
    filterKeys: ['cleanseCriteria', 'getSetMembers', function(next, results) {
      ...
      function search(member, done) {
        connection.client.get(member, function(err, result) {
