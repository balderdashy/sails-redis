...
var Indice = module.exports = function(collectionName, name, client) {
  ...
  this.keyName = 'waterline:' + name + ':_indicies:' + this.name;
...
Indice.prototype.index = function index(value, cb) {
  ...
  this._indexed(value, function(err, indexed) {
    ...
    if(indexed) return cb(Errors.adapter.notUnique);

    // Create the index
    self.sadd(self.keyName, value, function(err) {
      if(err) return cb(err);
      cb();
    });
  });
...
Indice.prototype._indexed = function _indexed(value, cb) {
  client.sismember(this.keyName, value, function(err, indexed) {
    if(err) return cb(err);
    cb(null, indexed);
  });
};
