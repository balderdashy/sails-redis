...
Schema.prototype.registerCollection = function(collectionName, schema, config, cb) {
  ...
  this._getConnection(name, config, function(err, connection) {
    ...
    for(var attr in schema) {
      if(Utils.object.hasOwnProperty(schema[attr], 'unique')) self._unique[name].push(attr);
      ...
      if(Utils.object.hasOwnProperty(schema[attr], 'unique')) {
        var indice = new Indice(name, attr, connection.client);
      ...
      if(Utils.object.hasOwnProperty(schema[attr], 'autoIncrement')) {
        var sequence = new Sequence(name, attr, connection.client);
