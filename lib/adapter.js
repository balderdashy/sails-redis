...
module.exports = {
  ...
  create: function(collection, data, callback) {
    ...
    transaction.exec(callback, function(done) {
      ...
      this.isUnique(collection, data, function(err, unique) {
        if(err) return done(err);
    ...
    function create(callback) {
      transaction.setIncrements(collection, data, function(err, data) {
        if(err) return callback(err);
        var key = transaction.recordKey(collection, primary,
          transaction.sanitize(data[primary]));

        redis.sismember(index, key, function(err, status) {
          if(err) return callback(err);
  ...
  find: function(collection, criteria, callback) {
    ...
    if(present(criteria[primary])) {
      transaction.exec(callback, function(done) {
        var redis = this.connection,
            recordKey = this.recordKey(collection, primary, criteria);
    ...
    transaction.exec(callback, function(done) {
      var recordKey,
          ...
          index = this.indexKey(collection, primary);
      ...
      function search(member, done) {
        redis.get(member, function(err, result) {
          ...
          if(match(result)) results.push(result);
          done();
        });
      }
    });
  ...
  update: function(collection, criteria, values, callback) {
    ...
    if(present(criteria[primary])) {
      transaction.exec(callback, function(done) {
        ...
        redis = this.connection;
        key = this.recordKey(collection, primary, criteria[primary]);
