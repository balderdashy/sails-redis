...
module.exports = (function () {
  ...
  registerCollection: function(collection, callback) {
    ...
    console.warn('The `sails-redis` adapter is currently under active development.');
    console.warn('It is currently for experimental use only.');

    // Configure transaction object
    _transaction.configure(config);
    _transaction.registerSchema(_schema);

    // Register with schema
    _schema.register(collection.identity, collection.definition);
  ...
  describe: function(collection, callback) {
    var desc = _schema.retrieve(collection);
  ...
  drop: function(collection, relations, callback) {
    var redis,
        schema = _schema,
